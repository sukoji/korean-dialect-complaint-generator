...
실행:  python tools/renumber_scenarios.py [CSV 경로 또는 결과 폴더]
       (인자 없으면 results/ 안 최신 comparison_*.csv 사용)
"""
import csv
import glob
import os
...
def renumber_by_region(rows):
    """지역(region_name)별로 원래 scenario_id 순서를 유지한 채 1..N 재부여."""
    by_region = {}
    for r in rows:
...
def main():
    ...
    for region, n in sorted(by.items()):
        print(f"    {region}: {n}개, scenario_id 1~{n}")
